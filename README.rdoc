...
Use the #request method to set a custom request handler path:
...
As of version 0.9.1, RSolr can now use LibXml to create the update messages sent to solr. To switch from Builder to LibXml, set the RSolr::Message.builder like so:
...
Example:
  ...
  solr.connect(:adapter => :curb)
  solr.connect(:adapter => :net_http)
