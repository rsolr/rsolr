...
NOTE: Please consider using the "pure" branch. This branch will soon become the master. The main differences are:
  - no tricky param mapping
  - much simplified code base
  - ability to set the request handler paths whenever a call is made to solr.

Param mapping will be wrapped up in a different library/extension.
