...
==Degradation
You can set the Rsolr.client to not raise exceptions when there is issue with the Solr endpoint. An empty set will be returned, with a 500 status in this case.

```ruby
  solr = RSolr.connect :url => 'http://solrserver.com'
  solr.raise_connection_exceptions = false
```

==Default Request Method
The client allows a default HTTP method to be set to either GET, or POST. All requests passed to the Solr endpoint will use this.

```ruby
  solr = RSolr.connect :url => 'http://solrserver.com'
  solr.request_method = :post
```
