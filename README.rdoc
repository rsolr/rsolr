...
== Querying
Use the #get, #post method to send requests to the /select handler:
...
nil is passed in as the query string data. The ":data => enormous_params_hash" ends up serialized as a form-encoded query string, and the correct content-type headers are sent along to Solr.
...
==NOOP
You can get the request context that's sent to Solr without sending it if you set :noop => true.
