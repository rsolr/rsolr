...
Use the #request method for a custom request handler path:
  response = solr.get 'documents', :q=>'test'
...
nil is passed in as the query string data. The enormous_params_hash variable ends up serialized into a form-encoded query string, and the correct content-type headers are sent along to Solr.
...
You can access the original request context (path, params, url etc.) by calling the #original_request method:
  response = solr.get 'select', :q=>'*:*'
  response.original_request

Similarly, the object returned has an original_response object. This contains any headers that Solr returned, along with the raw response body.
